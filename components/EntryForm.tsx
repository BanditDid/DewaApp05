...
interface EntryFormProps {
  ...
  birthDate: string;
  onSave: (entry?: JournalEntry) => void; // แก้ไข: ไม่จำเป็นต้องส่ง Entry กลับ เพราะโหลดใหม่จาก Sheet
  onCancel: () => void;
}
...
export const EntryForm: React.FC<EntryFormProps> = ({ initialData, birthDate, onSave, onCancel }) => {
  ...
  useEffect(() => {
    // ใช้ GoogleService แทน MockService
    GoogleService.getTags().then(setAvailableTags);
  ...
  const handleAddTag = () => {
    if (newTagInput.trim() && !availableTags.includes(newTagInput.trim())) {
      ...
      // หมายเหตุ: ใน GoogleService อาจจะยังไม่บันทึกทันทีก็ได้ แต่เพื่อความง่ายเราเรียก save ไว้ก่อน
      // GoogleService.saveTags(updatedTags); 
      setNewTagInput('');
    }
  };
  ...
  const removePhoto = (id: string) => {
    // ถ้าเป็นรูปที่เพิ่งเพิ่ม (ยังไม่มีใน Drive) ให้ลบออกจาก newPhotoFiles ด้วย
    if (id.startsWith('temp-')) {
        // อันนี้ซับซ้อนนิดหน่อย เพราะเราแมป index ยาก ให้รีเซ็ต newPhotoFiles ใหม่หมดเลยถ้ายกเลิก
        // เพื่อความง่ายใน demo: การลบ preview อาจจะไม่ลบ file ใน array จริง (ซึ่งเป็น bug เล็กน้อย)
        // แก้ไข: ในโค้ดจริงควรจัดการ state ให้แม่นยำกว่านี้ แต่สำหรับ MVP ให้กรองออกจาก photos (preview) ก็พอ
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ส่งข้อมูล + ไฟล์รูปใหม่ไปให้ GoogleService จัดการ
      await GoogleService.saveEntry({
        id: initialData?.id,
        date,
        notes,
        tags: selectedTags,
      ...
      await GoogleService.saveTags(availableTags);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 font-sans">
      ...
      <div>
        ...
        <p className="text-xs text-gray-400">*รูปภาพจะถูกอัปโหลดไปยัง Google Drive ของคุณ (ใช้เวลาสักครู่)</p>
      </div>
